...
var root_2 = add_locations(template(`<div class="fixed bottom-5 left-5 text-xs text-red-500">Tailwind CSS not detected</div>`), TailwindScreenSize$1[FILENAME], [[232, 2]]);
...
function TailwindScreenSize$1($$anchor, $$props) {
	...
	push($$props, false, TailwindScreenSize$1);

	const shouldShow = mutable_state();
	const activeBreakpoints = mutable_state();
	const containerClassNames = mutable_state();
	const textClassNames = mutable_state();
	const dividerClassNames = mutable_state();
	const breakpointClassNames = mutable_state();
	...
	let containerClassName = prop($$props, "containerClassName", 8, '');
	let textClassName = prop($$props, "textClassName", 8, '');
	let dividerClassName = prop($$props, "dividerClassName", 8, '');
	let breakpointClassName = prop($$props, "breakpointClassName", 8, '');
	let breakpoints = prop($$props, "breakpoints", 8, undefined);
	let showDefaultBreakpoints = prop($$props, "showDefaultBreakpoints", 8, true);
	let hideNoTailwindCSSWarning = prop($$props, "hideNoTailwindCSSWarning", 8, false);
	// State
	let currentBreakpoint = mutable_state('');
	let width = mutable_state(0);
	let isTailwindDetected = mutable_state(true);
	...
	function handleResize() {
		...
		set(currentBreakpoint, getCurrentBreakpoint(get(width), breakpoints() || defaultBreakpoints));
	}

	// Lifecycle
	onMount(() => {
		set(mounted, true);
		set(isTailwindDetected, detectTailwind());
		handleResize();
	...
	legacy_pre_effect(() => (deep_read_state(show())), () => {
		set(shouldShow, strict_equals(show(), null, false) && strict_equals(show(), void 0, false) ? show() : isDevelopment());
	});

	legacy_pre_effect(
		() => (
			deep_read_state(breakpoints()),
			deep_read_state(showDefaultBreakpoints())
		),
		() => {
			set(activeBreakpoints, breakpoints() || (showDefaultBreakpoints() ? defaultBreakpoints : []));
		}
	);

	legacy_pre_effect(
		() => (
			deep_read_state(theme()),
			deep_read_state(position()),
			deep_read_state(containerClassName()),
			deep_read_state(className())
		),
		() => {
			set(containerClassNames, twMerge(baseClasses.container, themeClasses[theme()].container, positionClasses[position()], containerClassName(), className()));
		}
	);
	...
	legacy_pre_effect(
		() => (
			deep_read_state(theme()),
			deep_read_state(textClassName())
		),
		() => {
			set(textClassNames, twMerge(themeClasses[theme()].text, textClassName()));
		}
	);
	...
	legacy_pre_effect(
		() => (
			deep_read_state(theme()),
			deep_read_state(dividerClassName())
		),
		() => {
			set(dividerClassNames, twMerge(baseClasses.divider, themeClasses[theme()].divider, dividerClassName()));
		}
	);
	...
	legacy_pre_effect(
		() => (
			deep_read_state(theme()),
			deep_read_state(breakpointClassName())
		),
		() => {
			set(breakpointClassNames, twMerge(themeClasses[theme()].breakpoint, breakpointClassName()));
		}
	);
